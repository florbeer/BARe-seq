...
#Import parameter values of the different enhancer libraries (STARR-seq experiments) contained in Enhancer_burst_parameters_NB.xls
#In this script we exclude the library with inserted motifs
samples <- excel_sheets("Enhancer_burst_parameters_NB.xls") #adjust path if required
data_list <- lapply(samples, function(y) {
  y2 <- read_excel("Enhancer_burst_parameters_NB.xls", sheet = y) %>%
    as.data.frame()
  rownames(y2) <- y2[,1]
  y2 <- y2[,2:ncol(y2)]
  return(y2)
...
finding_mean_thres(log2mu_list[[2]], low=-4, high=-1.5)
finding_mean_thres(log2mu_list[[3]], low=-4, high=-1.5)
...
#libFL004_STARR_DSCP			-2.8			0.3
#libFL005_STARR_Rpl35			-2.6			0.3
#libFL006_STARR_Rps15A			-2.4			0.4

#Go back to the corresponding datasets, and annotate which mena expression values are higher than the thresholds
#data_list <- lapply()
mean_thres <- c(-2.8, -2.6, -2.4) %>% rep(., each=3)
...
library(WriteXLS)
WriteXLS(data_list, ExcelFileName = "Enhancer_burst_parameters_NB_&_muthres.xls", SheetNames =names(data_list),row.names=TRUE)
