...
#STAP-seq experiments

library(dplyr)
library(readxl)

#Import parameter values of the different promoter libraries (STAP-seq experiments) contained in Promoter_burst_parameters_NB.xls
samples <- excel_sheets("Promoter_burst_parameters_NB.xls") #adjust path if required
data_list <- lapply(samples, function(y) {
  y2 <- read_excel("Promoter_burst_parameters_NB.xls", sheet = y) %>%
    as.data.frame()
  rownames(y2) <- y2[,1]
  y2 <- y2[,2:ncol(y2)]
  return(y2)
...
finding_mean_thres(log2mu_list[[1]], low=-5, high=-1.5)
finding_mean_thres(log2mu_list[[2]], low=-5, high=-1.5)
finding_mean_thres(log2mu_list[[3]], low=-5, high=-1.5)
...
#libFL001_STAP_zfh1			-3.5			0.5
#libFL002_STAP_CG4822			-4.1			0.4
#libFL003_STAP_Pld			-3.7			0.4


#Go back to the corresponding datasets, and annotate which mena expression values are higher than the thresholds
#data_list <- lapply()
mean_thres <- c(-3.5, -4.1, -3.7) %>% rep(., each=3)
...
library(WriteXLS)
WriteXLS(data_list, ExcelFileName = "Promoter_burst_parameters_NB_&_muthres.xls", SheetNames =names(data_list),row.names=TRUE)
