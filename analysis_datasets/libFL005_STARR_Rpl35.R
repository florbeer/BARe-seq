...
#Subset to libFL013
f <- grepl("libFL013", x$sample)
x <- x[f,]

x$sample2 <- x$sample
x$sample2 <- gsub(".+rep[1|2]", "pair1", x$sample)
x$sample2 <- gsub(".+rep[5|6]", "pair1", x$sample)
...
#Export the files
library(WriteXLS)
WriteXLS(dataX, ExcelFileName = "E39_lib013_burst_parameters_NB.xls", SheetNames =names(dataX),row.names=TRUE)
