...
#Import data
x <- read.csv("E24_STARR_counts.txt",sep="\t",header=TRUE)

samples <- unique(x$sample) %>% sort()
aL <- split(x, x$sample)

samples2 <- grep("libFL010", samples, value=TRUE)
aL <- aL[samples2]

#Add an extra slot with the pooled sample
f <- grepl("libFL010", x$sample) & grepl("r(1|2)r", x$sample)
aL$pooled <- x[f,]
...
#Export the files
library(WriteXLS)
WriteXLS(dataX, ExcelFileName = "E24_burst_parameters_NB.xls", SheetNames =names(dataX),row.names=TRUE)
