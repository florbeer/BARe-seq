...
#Import data
x <- read.csv("E57_STARR_v2_processing1_counts.txt",sep="\t",header=TRUE)

#Subset to libFL031
f <- grepl("libFL031", x$sample)
x <- x[f,]

x$sample2 <- x$sample
x$sample2 <- gsub(".+rep[1|2]", "pair1", x$sample)
x$sample2 <- gsub(".+rep[3|4]", "pair2", x$sample)

samples <- unique(x$sample2) %>% sort()
aL <- split(x, x$sample2)


samples2 <- grep("pair", samples, value=TRUE)
aL <- aL[samples2]

#Add an extra slot with the pooled sample
f <- grepl("pair", x$sample2)
aL$pooled <- x[f,]
...
#Export the files
library(WriteXLS)
WriteXLS(dataX, ExcelFileName = "E57_lib031_burst_parameters_NB.xls", SheetNames =names(dataX),row.names=TRUE)
