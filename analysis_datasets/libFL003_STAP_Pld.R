...
samples2 <- grep("libFL017", samples, value=TRUE)
aL <- aL[samples2]

#Add an extra slot with the pooled sample
f <- grepl("libFL017", x$sample) & grepl("rep(5|6)", x$sample)
aL$pooled <- x[f,]
...
#Export the files
library(WriteXLS)
WriteXLS(dataX, ExcelFileName = "E38_lib017_burst_parameters_NB.xls", SheetNames =names(dataX),row.names=TRUE)
